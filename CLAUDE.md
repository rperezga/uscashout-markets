...
| `public/mobile/index.html` | **v2.9** Shell de la versión MÓVIL: login a pantalla completa, vistas Portafolio y Cuenta, bottom-nav | ~150 líneas |
| `public/mobile/mobile.js` | **v2.9** Lógica móvil: auth, portafolio, gráfica de rendimiento, idioma. Consume los mismos endpoints que el escritorio | ~450 líneas |
| `public/mobile/mobile.css` | **v2.9** Estilos mobile-first (safe-area, targets ≥44px). Independiente de `style.css` | ~400 líneas |
...
## Versión web vs versión móvil (v2.9 — leer antes de tocar el frontend)

- **Dos frontends separados, un solo backend.** Escritorio: `public/index.html` + `app.js` + `style.css` (dashboard completo). Móvil: `public/mobile/*` (3 tabs: Mi Portafolio, Monedas y Cuenta). Comparten servidor, endpoints y sesión: el móvil NO duplica lógica de negocio, solo consume `/api/auth/*`, `/api/settings`, `/api/portfolio`, `/api/portfolio/history`, `/api/coins/summary` y `/api/coins/refresh`. Si cambias un cálculo, se cambia en el servidor y lo ven los dos.
- **Tab "Monedas" (v2.9)**: lista de las 9 monedas del registro `COINS` y, al tocar una, su ficha (precio/24h, score, veredicto del `dailyBrief`, niveles y las señales que lo sustentan). Lista y detalle son dos estados de la MISMA vista (`#view-coins`), no dos entradas del bottom-nav: el detalle vuelve con su propia flecha.
- **`GET /api/coins/summary`**: lo esencial de cada moneda en una respuesta. Existe porque `/api/data` devuelve el estado completo (cientos de KB) y el móvil solo necesita una foto. Ojo al leerlo: XRP es legacy y sus nodos viven en la RAÍZ del estado, el resto en `coins.<id>`.
- **`POST /api/coins/refresh`**: refresca una moneda fría SIN tocar el ajuste `activeCoin`. No uses `/api/coins/activate` desde el móvil: aquel persiste la moneda activa de la cuenta y haría que el escritorio saltara a la última moneda mirada en el teléfono. Lleva cooldown de 60 s por moneda contra estampidas.
- **Qué versión se sirve**: lo decide `app.get('/')` vía `resolveView(req)` — cookie `viewMode` (elección explícita) por encima del User-Agent. Las tablets van a escritorio a propósito. Rutas de escape: **`/m`** fuerza móvil y **`/desktop`** fuerza escritorio (fijan la cookie y redirigen a `/`). Para previsualizar el móvil desde un PC no hace falta un teléfono: abre `/m`.
- **Barras de escape entre versiones (v2.9), no las quites**: la cookie `viewMode` dura 180 días y gana al User-Agent, así que elegir una versión te deja en ella — pasó de verdad: abrir `/m` una vez desde un portátil dejó el escritorio inaccesible. Por eso cada versión enseña la salida a la otra **solo cuando estás en el estado forzado**: `setupViewBar()` en `mobile.js` muestra una barra si existe `viewMode=mobile` (un teléfono normal llega por UA, sin cookie, y no la ve), y el bloque `mobileEscapeBar()` al final de `app.js` muestra la inversa si existe `viewMode=desktop` **y** la pantalla mide menos de 820 px. Si añades otro modo de vista, añade su salida.
- **`express.static` va con `index:false`**. Sin eso servía `public/index.html` en `/` antes de llegar al router de vista, y `app.get('/')` quedaba muerto. Si alguien lo revierte, el móvil deja de aparecer sin dar ningún error.
- **Rutas de assets ABSOLUTAS en el HTML móvil** (`/mobile/mobile.css`, `/i18n.js`, `/vendor/...`): ese HTML se sirve tanto en `/` como en `/mobile/index.html`; con rutas relativas se rompería en `/`.
- **Añadir una vista móvil nueva**: un `<button data-view="X">` en el `<nav class="m-nav">` + una `<section id="view-X" class="m-view">`. `showView()` hace el resto; no hay que tocar nada más. Mantén el alcance corto a propósito: el móvil es para mirar, no para administrar.
- **i18n compartido**: el móvil carga el mismo `public/i18n.js`. Lo exclusivo suyo va en el namespace `m.*`; todo lo que ya existe (`auth.*`, `acc.*`, `nav.portfolio`, `btn.salir`) se reusa. Incrementa `?v=` de `i18n.js` en LOS DOS HTML al tocarlo.
- El móvil tolera el 503 de arranque (`code:'STARTING'`) mostrando "Arrancando el servidor…" y reintentando: sin eso aparecía un login que rechazaba credenciales correctas mientras Mongo conectaba.
...
- **El P&L de 24h usa esa MISMA fórmula (v2.9)**, no la serie diaria. Antes `pnl.d1` salía de comparar el valor de hoy contra el snapshot del día anterior (medida por fecha), mientras el titular usaba el cambio rodante de 24h de CoinGecko: dos cosas distintas con la misma etiqueta, que en el móvil quedaban a tres centímetros y se contradecían (−9,40% junto a −0,01%). `_currentPortfolioTotal()` devuelve ahora `change24hValue`/`change24hPct` y `pnl.d1` los usa (con `source:'rolling24h'`); 7d/30d/desde-inicio siguen saliendo del histórico diario, que para esos plazos sí es lo correcto. De paso, esa función pedía `/simple/price` SIN `include_24hr_change` mientras `/api/portfolio` lo pedía CON él **compartiendo la misma caché**: ahora ambas piden lo mismo.
