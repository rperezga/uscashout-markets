...
- **Dos frontends separados, un solo backend.** Escritorio: `public/index.html` + `app.js` + `style.css` (dashboard completo). Móvil: `public/mobile/*` (login + Mi Portafolio + Cuenta). Comparten servidor, endpoints y sesión: el móvil NO duplica lógica de negocio, solo consume `/api/auth/*`, `/api/settings`, `/api/portfolio` y `/api/portfolio/history`. Si cambias un cálculo, se cambia en el servidor y lo ven los dos.
